...
namespace HOK.SmartBCF.GoogleUtils
{
    public static class FileManager
    {
        ...
        public static void ReadAppData()
        {
            ...
            {
                MessageBox.Show("Read configuration in app folder.\n"+ex.Message, "Read App Data", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }

        public static BCFFolderStructure CreateBcfFolders(ModelInfo info, SmartBCFConfiguration config)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(info.GoogleFolderTitle))
                    {
                        string folderLocation = "";
                        if (info.ExternalUser)
                        {
                            folderLocation = "External Users";
                        }
                        else
                        {
                            if (info.HOKStandard)
                            {
                                folderLocation = info.FileLocation;
                            }
                            else
                            {
                                folderLocation = info.UserLocation;
                            }
                        }

                        var parentFolders = from folder in projectFolders where folder.FolderName == folderLocation select folder;
                        if (parentFolders.Count() > 0)
                        {
                            GoogleFolderInfo parentFolderInfo = parentFolders.First();
                            string projectFolderTitle = info.GoogleFolderTitle;

                            Permission permission = new Permission();
                            permission.Value = "";
                            permission.Type = "anyone";
                            permission.Role = "writer";
                            permission.WithLink = false;

                            File body = new File();
                            body.Title = projectFolderTitle;
                            body.MimeType = "application/vnd.google-apps.folder";
                            body.Parents = new List<ParentReference>() { new ParentReference() { Id = parentFolderInfo.FolderId } };

                            FilesResource.InsertRequest request = service.Files.Insert(body);
                            File createdFolder = request.Execute();

                            Permission insertedPermission = service.Permissions.Insert(permission, createdFolder.Id).Execute();

                            File newFolder = service.Files.Get(createdFolder.Id).Execute();
                            string webLink = newFolder.WebViewLink;
            ...
            {
                MessageBox.Show("Failed to create bcf folders.\n"+ex.Message, "Create BCF Folders", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        ...
        public static BCFFolderStructure CreateDefaultFolders(File projectFolder)
        {
            BCFFolderStructure bcfFolders = new BCFFolderStructure();
            try
            {
                if (null == service)
                {
                ...
                if (null != service)
                {
                    bcfFolders.ProjectId = projectFolder.Id;
                    bcfFolders.ProjectName = projectFolder.Title;
                    bcfFolders.ProjectLink = projectFolder.WebViewLink;

                    File colorSheet = CreateColorSheet(projectFolder.Id);
                    bcfFolders.ColorSheet = new GoogleFileInfo(colorSheet, BCFFileType.Spreadsheet);

                    //active folders
                    File activeFolder = CreateSubFolder(projectFolder.Id, "Active", "Interactive BCF Data will be stored in this folder.");
                    bcfFolders.ActiveBCFFolder = new GoogleFolderInfo(activeFolder.Title, activeFolder.Id);
                    File activeBcfFolder = CreateSubFolder(activeFolder.Id, "BCF_Files", "Parsed BCF Data will be stored as Google Spreadsheet.");
                    bcfFolders.ActiveBCFFolder = new GoogleFolderInfo(activeBcfFolder.Title, activeBcfFolder.Id);
                    File activeImgFolder = CreateSubFolder(activeFolder.Id, "BCF_Images", "Screen captured images of each issue will be stored in this folder.");
                    bcfFolders.ActiveImageFolder = new GoogleFolderInfo(activeImgFolder.Title, activeImgFolder.Id);

                    //archive folders
                    File archiveFolder = CreateSubFolder(projectFolder.Id, "Archive", "Archived .bcfzip files will be stored in this folder.");
                    bcfFolders.ArchiveFolder = new GoogleFolderInfo(archiveFolder.Title, archiveFolder.Id);
                    File archiveBcfFolder = CreateSubFolder(archiveFolder.Id, "BCF_Files", "Archived bcfzip file will be stored in this folder");
                    bcfFolders.ArchiveBCFFolder = new GoogleFolderInfo(archiveBcfFolder.Title, archiveBcfFolder.Id);
                    File archiveImgFolder = CreateSubFolder(archiveFolder.Id, "BCF_Images", "Archived image files will be stored in this folder.");
                    bcfFolders.ArchiveImageFolder = new GoogleFolderInfo(archiveFolder.Title, archiveFolder.Id);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Failed to create default bcf folders.\n"+ex.Message, "Create Default Folders", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
            return bcfFolders;
        }
