...
namespace HOK.ClipboardMonitor
{
    class Program
    {
        ...
        {
            Console.WriteLine("Clipboard Monitor has been activated..");
            Console.WriteLine("Close this window to exit this tool.");
            Start();
            ...
            private void ClipChanged()
            {
                ...
                try
                {
                    ...
                    if (format == ClipboardFormat.Text)
                    {
                        ...
                        {
                            string copiedText = Clipboard.GetText(TextDataFormat.Rtf);
                            if (copiedText.Contains("\\par\r\n") || copiedText.Contains("\\par "))
                            {
                                string modifiedText = copiedText.Replace("\\par\r\n", "");
                                modifiedText = modifiedText.Replace("\\par ", " ");
                                Clipboard.SetText(modifiedText, TextDataFormat.Rtf);
                            }
                        }
                    }
