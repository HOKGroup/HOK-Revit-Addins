...
namespace HOK.ModelManager.GoogleDocs
{
    public class GoogleDocsUtil
    {
        ...
        public bool DeleteLinkInfo(SpreadsheetEntry sheetEntry, List<LinkInfo> linkInfoList, Dictionary<int, ViewProperties> removeMissingItems)
        {
            bool result = false;
            try
            {
                if (null != sheetEntry)
                {
                    ...
                    foreach (int sourceViewId in removeMissingItems.Keys)
                    {
                        try
                        {
                            listQuery.SpreadsheetQuery = "itemsourceid=" + sourceViewId;
                            ListFeed listFeed = sheetService.Query(listQuery);
                            if (listFeed.Entries.Count > 0)
                            {
                                ListEntry row = (ListEntry)listFeed.Entries[0];
